...
def getBigRandomString():
    return str(
        random.randint(
            random.randint(
                round(time.time() + 2), round(time.time()) + round(time.time() + 8)
            ),
            round(time.time() - 3) * round(time.time()) + 1,
        )
    )
...
def sendEmail(receiver, subject, userRec, reason, MsgType="html"):
    ...
    reason: 1 = Bienvenida, 2 = recuperar contraseña via telefóno, 3 = recuperar contraseña via correo, 4 = Envío de solicitud de amistad, 5 = Envio de invitación a grupo
    """
    try:
        ...
        if reason == "1":
            ...
            UTemplate.info = {"name": userRec["name"]}
            message = UTemplate.recoveryPwdHtml()
        elif reason == "3":
            UTemplate.info = {"code": userRec["code"], "name": userRec["name"]}
            ...
            message = UTemplate.sendQueryGroup()
        else:
            return False
...
def decodeAuth2CertGoogleAPI_GO(id_token):
    try:
        verify = requests.get(
            "https://www.googleapis.com/oauth2/v3/tokeninfo?id_token={}".format(
                id_token
            )
        ).json()
...
def sendSMS(number, msg):
    try:
        ...
        RSend = requests.get(
            url=API_AUTH2_SMS_env + "&message=" + msg + "&numero=" + number
        )
...
def initChat(id_emisor=None, id_evento_grupal=None, typeChat=None, id_receptor=None):
    try:
        ...
        if typeChat == "2":
            verificarGrupo = dataTableMysql(
                "SELECT tipo_ev FROM eventos WHERE id = '{}'".format(id_evento_grupal)
            )
            ...
            verificarAmistad = dataTableMysql(
                "SELECT id_usuario, id_contacto, estado_invitacion FROM contactos WHERE (id_usuario = '{}' OR id_contacto = '{}') AND (id_usuario = '{}' OR id_contacto = '{}')".format(
            ...
            for item in verificarAmistad:
                if (
                    str(item[0]) == str(id_emisor)
                    and str(item[1]) == str(id_receptor)
                    or str(item[1]) == str(id_emisor)
                    and str(item[0]) == str(id_receptor)
                ):
